...
@extend_schema(
    ...
    responses={200: OpenApiResponse(description="Activation email")},
)
@api_view(["GET"])
...
@extend_schema(
    ...
    responses={201: OpenApiResponse(description="Utilisateur créé")},
)
@api_view(["POST"])
@permission_classes([AllowAny])
def register_view(request):
    ...
    if serializer.is_valid():
        user = serializer.save(is_active=True) # true for now set to false after configuring email validation
...
@extend_schema(
    ...
    responses={200: OpenApiResponse(description="Connecté")},
)
@api_view(["POST"])
@permission_classes([AllowAny])
...
# logout user
@extend_schema(responses={200: OpenApiResponse(description="Déconnecté")})
@api_view(["POST"])
...
@extend_schema(responses={200: OpenApiResponse(description="Utilisateur supprimé")})
@api_view(["DELETE"])
@permission_classes([IsAuthenticated])
